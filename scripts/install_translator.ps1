...
if ($LASTEXITCODE -ne 0) {
    Write-Warning "Standard pip install failed; retrying with trusted PyPI hosts."
    & $pythonPath -m pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org "argostranslate==1.9.6" "beautifulsoup4==4.12.3"
}
if ($LASTEXITCODE -ne 0) {
...
if match is None:
    raise SystemExit('No Argos package found for {}->{}'.format(source, target))
path = match.download()
